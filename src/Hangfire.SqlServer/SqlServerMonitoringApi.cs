...
namespace Hangfire.SqlServer
{
    internal class SqlServerMonitoringApi : IMonitoringApi
    {
        ...
        {
            return UseConnection((connection, transaction) => 
                GetNumberOfJobsByStateName(connection, transaction, new Dictionary<string, string> { { "stateName", FailedState.StateName } }));
        }

        public long ProcessingCount()
        {
            return UseConnection((connection, transaction) => 
                GetNumberOfJobsByStateName(connection, transaction, new Dictionary<string, string> { { "stateName", ProcessingState.StateName } }));
        }
...
select * from [{_storage.SchemaName}].State where JobId = @id order by Id desc";
        ...
        {
            return UseConnection((connection, transaction) => 
                GetNumberOfJobsByStateName(connection, transaction, new Dictionary<string, string> { { "stateName", SucceededState.StateName } }));
        }

        public long DeletedListCount()
        {
            return UseConnection((connection, transaction) => 
                GetNumberOfJobsByStateName(connection, transaction, new Dictionary<string, string> { { "stateName", DeletedState.StateName } }));
        }
...
where j.Id in @jobIds";
        ...
        private long GetNumberOfJobsByStateName(DbConnection connection, DbTransaction transaction, Dictionary<string, string> parameters)
        {
            ...
            var sqlQuery = _jobListLimit.HasValue
                ? $@"select count(j.Id) from (select top (@limit) Id from [{queryParams[0]}].Job where StateName = @state {queryParams[1]} {queryParams[2]} {queryParams[3]}) as j"
                : $@"select count(Id) from [{queryParams[0]}].Job where StateName = @state {queryParams[1]} {queryParams[2]} {queryParams[3]} ";
...
$@"select * from(
  ...
  from [{queryParams[0]}].Job j with (forceseek)
  left join [{queryParams[0]}].State s on j.StateId = s.Id
